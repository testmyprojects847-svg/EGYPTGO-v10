...
export const bookingService = {
  ...
  create(tour: Tour, user: Pick<User, 'name' | 'email'>, date: string, travelers: number, locale: Locale = 'en', country?: Country): Booking {
...
export function exportBookingsCsv(bookings: Booking[]) {
  const header = ['Reference', 'Customer', 'Email', 'Country', 'Tour', 'Date', 'Travelers', 'Total', 'Currency', 'Status']
  const rows = bookings.map((b) => [b.reference ?? b.id, b.customerName, b.customerEmail, b.countryNameEn ?? b.countryCode ?? '', b.tourTitle, b.date, b.travelers, b.total, b.currency ?? 'USD', b.status])
